...
$weight = $groups -> getSubLastWeight(0, "Region");
...
$parentId = $_GET['parentId'];
	...
	if(isset($_POST['save']) && $_POST['save'] == "Save")
	{
		...
		if (empty($msg)) {
			$catId = $groups->saveRegion('', $name, $urlname, $block, $shortcontents, $contents, $weight, $pageTitle, $pageKeyword);
	...
	if(isset($_POST['edit']) && $_POST['edit'] == "Edit")
	{
		...
		if (empty($msg)) {
			$groups->saveRegion($_GET['eid'], $name, $urlname, $block, $shortcontents, $contents, $weight, $pageTitle, $pageKeyword);
			$groups->saveImage($_GET['eid']);
			$msg="Region Updated Successfully";
			
			header("Location: region.php?action=edit&eid=".$_GET['eid']."&msg=$msg");
			exit();
		}
		else {
			header("Location: region.php?action=edit&eid=".$_GET['eid']."&msg=$msg");
...
if (isset($_GET['deleteImage'])) {
	...
	header ("Location: region.php?action=edit&eid=".$_GET['id']."&msg=$msg");
}
?>
<?
function createDrpMenu($catId, $startDash)
{
	...
	$catResult = $groups->getByParentIdAndLinkType($catId, "Region");
...
function createDrpMenu1($catId, $startDash)
{
	...
	{
	?>
    <option value="region.php?parentId=<?= $catRow['id']?>" <? if($_GET['parentId'] == $catRow['id']){?> selected<? }?>>
    <?= $dash.$catRow['name'];?>
    </option>
...
<form action="<?= $_REQUEST['uri'];?>" method="post" onsubmit="return frmvalidate(this)" enctype="multipart/form-data">
        <table width="100%" border="0" cellspacing="2" cellpadding="0">
          <tr>
            <th colspan="2" class="heading2">Manage Region
            <a href="region.php" style="float:right; padding-right:10px; text-decoration:none; color:#cc0000;">Add New</a></th>
          </tr>
          <tr>
            <th colspan="1">Add Region</th>
          </tr>
          
          <tr>
            <th class="tahomabold11">Region Name <span class="asterisk">*</span></th>
            <td>
...
?>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <th colspan="2" class="heading2">Manage Region <a href="region.php" style="float:right; padding-right:10px; text-decoration:none; color:#cc0000;">Add New</a></th>
                </tr>
                <tr>
                  <th colspan="1">Edit Region</th>
                </tr>
                <?
               ...
               <tr>
                <th class="tahomabold11">Region Name</th>
                <td>
              ...
              <?php
                ...
                <tr>
                  ...
                  [<a href="region.php?id=<?=$row['id'];?>&deleteImage">Delete</a>]</td>
                </tr>
                <?php } ?>
                <tr><td></td></tr>
                <tr>
    ...
    <?
			if($parentId == 0)
			...
			else
				{
					...
					echo "<a href=\"region.php?parentId=".$selRow['parentId']."\" style=\"float:right; color:#cc0000; text-decoration:none; padding-right:10px;\">UP</a>";
				}
		?>
    </th>
    </tr>
  <tr bgcolor="#E7E7E7" class="tahomabold11">
   ...
   <?
	 $counter = 0;
		$catResult = $groups->getByParentIdAndLinkType($parentId, "Region");
			while($catRow = $conn->fetchArray($catResult))
				{
					$counter++;
	?>
    ...
    <td>
			...
			<? 	$categoryResult = $groups->getByParentId($catRow['id']);
				...
				if($count < 1 && $count1 < 1) {?>
                     ...
                     <? }?> <a href="region.php?action=edit&eid=<?= $catRow['id'];?>">Edit</a>
                     | <a href="#" onClick="javascript: if(confirm('This will permanently remove this Region from database. Continue?')){ document.location='region.php?action=delete&did=<?= $catRow['id']?>'; }">Delete</a></td>
  </tr>
	<?
		}
	?>
