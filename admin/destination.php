...
$weight = $groups -> getSubLastWeight(0, "Destination");
...
$parentId = $_GET['parentId'];
	...
	if(isset($_POST['save']) && $_POST['save'] == "Save")
	{
		...
		if (empty($msg)) {
			$catId = $groups->saveDestination('', $name, $urlname, $block, $shortcontents, $contents, $weight, $pageTitle, $pageKeyword);
			$groups->saveImage($catId);
			
			$msg="Destination Added Successfully";
			
			header("Location: destination.php?msg=$msg");
			exit();
		}
		else {
			header("Location: destination.php?msg=$msg");
	...
	if(isset($_POST['edit']) && $_POST['edit'] == "Edit")
	{
		...
		if (empty($msg)) {
			$groups->saveDestination($_GET['eid'], $name, $urlname, $block, $shortcontents, $contents, $weight, $pageTitle, $pageKeyword);
			$groups->saveImage($_GET['eid']);
			$msg="Destination Updated Successfully";
			
			header("Location: destination.php?action=edit&eid=".$_GET['eid']."&msg=$msg");
			exit();
		}
		else {
			header("Location: destination.php?action=edit&eid=".$_GET['eid']."&msg=$msg");
...
if (isset($_GET['deleteImage'])) {
	...
	header ("Location: destination.php?action=edit&eid=".$_GET['id']."&msg=$msg");
}
?>
<?
function createDrpMenu($catId, $startDash)
{
	...
	$catResult = $groups->getByParentIdAndLinkType($catId, "Destination");
...
function createDrpMenu1($catId, $startDash)
{
	...
	{
	?>
    <option value="destination.php?parentId=<?= $catRow['id']?>" <? if($_GET['parentId'] == $catRow['id']){?> selected<? }?>>
    <?= $dash.$catRow['name'];?>
    </option>
...
<script language="javascript">
	function frmvalidate(fm){
		if(fm.name.value == ""){
			alert("Please type Destination name.");
			fm.name.focus();
			return false;
		}	
...
<form action="<?= $_REQUEST['uri'];?>" method="post" onsubmit="return frmvalidate(this)" enctype="multipart/form-data">
        <table width="100%" border="0" cellspacing="2" cellpadding="0">
          <tr>
            <th colspan="2" class="heading2">Manage Destination
            <a href="destination.php" style="float:right; padding-right:10px; text-decoration:none; color:#cc0000;">Add New</a></th>
          </tr>
          <tr>
            <th colspan="1">Add Destination</th>
          </tr>
          
          <tr>
            <th class="tahomabold11">Destination Name <span class="asterisk">*</span></th>
            <td>
...
?>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <th colspan="2" class="heading2">Manage Destination <a href="destination.php" style="float:right; padding-right:10px; text-decoration:none; color:#cc0000;">Add New</a></th>
                </tr>
                <tr>
                  <th colspan="1">Edit Destination</th>
                </tr>
                <?
               ...
               <tr>
                <th class="tahomabold11">Destination Name</th>
                <td>
              ...
              <?php
                ...
                <tr>
                  ...
                  <td>
                  <img src="../data/imager.php?file=../<?= CMS_GROUPS_DIR . $row['image'];?>&mw=100&mh=100" border="0" alt="image" />
                  [<a href="destination.php?id=<?=$row['id'];?>&deleteImage">Delete</a>]</td>
                </tr>
                <?php } ?>
                <tr><td></td></tr>
                <tr>
    ...
    <?
			if($parentId == 0)
			...
			else
				{
					...
					echo "<a href=\"destination.php?parentId=".$selRow['parentId']."\" style=\"float:right; color:#cc0000; text-decoration:none; padding-right:10px;\">UP</a>";
				}
		?>
    </th>
    </tr>
  <tr bgcolor="#E7E7E7" class="tahomabold11">
   ...
   <?
	 $counter = 0;
		$catResult = $groups->getByParentIdAndLinkType($parentId, "Destination");
			while($catRow = $conn->fetchArray($catResult))
				{
					$counter++;
	?>
  <tr align="left" <?php if ($counter % 2 == 0) { echo "bgcolor='#F1F1F1'";} ?>>
    ...
    <td><?=$catRow['block'];?></td>
    <td><?=$catRow['weight'];?></td>
    <td><img src="../<?=CMS_GROUPS_DIR.$catRow['image'];?>" width="55" height="40" /></td>
    <td>
			...
			<? 	$categoryResult = $groups->getByParentId($catRow['id']);
				...
				if($count < 1 && $count1 < 1) {?>
                     ...
                     <? }?> <a href="destination.php?action=edit&eid=<?= $catRow['id'];?>">Edit</a>
                     | <a href="#" onClick="javascript: if(confirm('This will permanently remove this Destination from database. Continue?')){ document.location='destination.php?action=delete&did=<?= $catRow['id']?>'; }">Delete</a></td>
  </tr>
	<?
		}
	?>
