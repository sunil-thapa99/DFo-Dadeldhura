...
if($_GET['type']=="status")
{ 
	$cdetail = $testimonials -> updateStatus($id);
...
{ 
	$cdetail = $testimonials -> getById($id);
}
?>
                          ...
                          <tr <?php if($counter%2 != 0) echo 'bgcolor="#F7F7F7"'; else echo 'bgcolor="#FFFFFF"'; ?>>
                            ...
                            <td valign="top">
														...
														if($row['status']==0)
														{
															echo "Inactive";
														?>
														<a href="testimonials.php?type=status&id=<?=$row['id']?>">[Enable]</a>
														<?php
														}
														else
														{
															echo "Active";
													 	?>
														<a href="testimonials.php?type=status&id=<?=$row['id']?>">[Disable]</a> 
														<?php
														}
														?>     
														&nbsp;</td>
                            <td valign="top">
														...
														?>														</td>
                            <td valign="top">
														[<a href="testimonials.php?type=show&id=<?php echo $row['id']; ?>">Details</a>]
														[<a href="#" onClick="javascript: if(confirm('This will permanently delete Testimonial details from database. Continue?')){ document.location='testimonials.php?type=del&id=<?php echo $row['id']; ?>'; }">Delete</a>]														</td>
                          </tr>
                          <?
													}
													?>
